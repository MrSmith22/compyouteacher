...
const BASE = process.env.WP095_BASE || 'http://127.0.0.1:3000';
const email = (env.DEV_AUTH_EMAIL || 'dev-student@localhost').trim().toLowerCase();
const out = '/tmp/wp095-browser';
...
(async () => {
  ...
  async function runViewport(width, height, tag) {
    ...
    // --- Modules 2–5, 7 (WP-095 generalization) ---
    const midRoutes = [
      {
        id: 'm2',
        path: '/modules/2/success',
        variant: 'phase_transition',
        titleRe: /evidence set is ready|Module 2/i,
        notRe: /argument is ready for planning|analysis is finished/i,
        ctaRe: /Continue to Module 3/i,
      },
      {
        id: 'm3',
        path: '/modules/3/success',
        variant: 'artifact_completed',
        titleRe: /argument map is ready|strengthening both works/i,
        notRe: /submitted|Google Doc ready/i,
        ctaRe: /Continue to Module 4|Review Module 3/i,
      },
      {
        id: 'm4',
        path: '/modules/4/success',
        variant: 'artifact_completed',
        titleRe: /writing plan is ready|Finish your paragraph plans/i,
        notRe: /outline is ready to draft/i,
        ctaRe: /Continue to Module 5|Review Module 4/i,
        expectPlan: true,
      },
      {
        id: 'm5',
        path: '/modules/5/success',
        variant: 'phase_transition',
        titleRe: /outline is ready|Finish your outline|Could not load your outline/i,
        notRe: /writing plan is ready to outline/i,
        ctaRe: /Continue to Module 6|Review Module 5|Try again/i,
        expectPlan: true,
      },
      {
        id: 'm7',
        path: '/modules/7/success',
        variant: 'phase_transition',
        titleRe: /revised essay is ready|Module 7/i,
        notRe: /submitted|APA formatting complete|Your paper was received/i,
        ctaRe: /Continue to Module 8/i,
      },
    ];

    for (const route of midRoutes) {
      await page.goto(`${BASE}${route.path}`, { waitUntil: 'domcontentloaded', timeout: 60000 });
      await page.waitForTimeout(3500);
      const shellCount = await page.locator('[data-testid="success-experience-shell"]').count();
      const bodyText = await page.innerText('body');
      const incompleteOnly =
        /Finish your|Could not load|not finalized|not ready/i.test(bodyText) && shellCount === 0;
      check(
        `${route.id}_shell_or_honest_${tag}`,
        shellCount > 0 || incompleteOnly,
        `shell=${shellCount}`
      );
      if (shellCount > 0) {
        const variant = await page
          .locator('[data-testid="success-experience-shell"]')
          .getAttribute('data-variant');
        check(`${route.id}_variant_${tag}`, variant === route.variant, variant || 'none');
        check(`${route.id}_copy_${tag}`, route.titleRe.test(bodyText), 'title/explanation');
        check(`${route.id}_honesty_${tag}`, !route.notRe.test(bodyText), 'no overclaim');
        check(`${route.id}_cta_${tag}`, route.ctaRe.test(bodyText), 'cta');
        if (route.expectPlan) {
          check(
            `${route.id}_plan_stage_${tag}`,
            /Plan/i.test(bodyText) && await page.locator('[data-testid="success-journey-trail"]').count() > 0,
            'plan trail'
          );
        }
        check(`${route.id}_overflow_${tag}`, !(await overflow(page)), 'no h-overflow');
      }
      await page.screenshot({ path: path.join(out, `${route.id}-${tag}.png`), fullPage: true });
    }
    ...
    if (hasReceipt) {
      ...
      check(`m9_fields_${tag}`,
        ...
        'fields');
      check(`m9_journey_${tag}`, (await page.locator('[data-testid="success-journey-trail"], [data-testid="module9-accomplishment-trail"]').count()) > 0 && /Understand|Prepare|Submit|Plan|Draft/i.test(t), 'journey');
