...
export default function ModuleNine() {
  ...
  // track that we've already logged module_started once
  ...
  const handleSubmit = async () => {
    ...
    if (session?.user?.email) {
      ...
      await logActivity(session.user.email, "quiz_submitted", {
        ...
        score: total,
        total: questions.length,
        // lightweight per-question correctness
  ...
  const handleExportToGoogleDocs = async () => {
    ...
    if (!text) {
      alert(
        "Could not find your essay text yet. Finish Modules 6–7, then try again."
      );
      return;
    }
  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      <h1 className="text-3xl font-extrabold text-theme-blue mb-2">
        📘 Module 9: APA Mini Quiz
      </h1>
        ...
        <div className="space-y-4">
            ...
            <div className="mt-4 border rounded p-3 bg-white shadow">
              ...
              {popupBlocked && (
                <p className="text-sm text-orange-700 mt-2">
                  It looks like a popup blocker stopped the new tab. Use the
                  link above, or allow popups for localhost.
                </p>
              )}
            </div>
          )}
          ...
          <section className="mt-8 border p-4 rounded bg-white shadow">
            <h2 className="text-lg font-semibold mb-2 text-theme-dark">
              📤 Submit Final Essay as PDF
            </h2>
            <p className="text-sm text-gray-600 mb-2">
              Download your Google Doc as a PDF and upload it here.
            </p>
