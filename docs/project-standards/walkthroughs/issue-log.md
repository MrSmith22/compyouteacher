...
**WP-095 evidence (2026-07-22):** Catalog `3-to-4` `actionLabel` aligned to product **`Start Body Paragraph 1`** (`MODULE4_HANDOFF_CTA_LABEL`). Modules 4–7 success now mount `SuccessExperienceShell`; WP-052 assertions verify catalog copy + CTA destinations still drive the shell (not stale ModuleRoleTransitionCard-only checks). Automated: `tests/wp052-module-role-transitions.test.js` green.
...
- **Independent re-verification (July 14, 2026, commit `5de04f9`):** Re-ran the full browser acceptance on the completed-Module-3 seed account against the single dev server on port 3000. In Module 2 confirmed the six-cell matrix complete, changed a rating (Speech logos 5→10), selected a different supported essay direction (logic similarity), saved the audience/purpose reasoning, and continued through `/modules/2/success` into Module 3. Module 3 resumed at the **Thesis** stage with the "Your Module 2 analysis changed…" review banner; existing idea/claim/thesis/proof-plan prose was preserved verbatim ("wording has been preserved"); "Finish your thesis and continue" was gated until handled; the retain/adopt path ("This still says what I mean") cleared the banner, ungated advancement, and stored the current matrix signature (`v1::meaningful_similarity:logos::…`); reload showed no false warning. Repeated at **390×844** with a second direction change (logic → largest-contrast pathos): banner + gate + signature storage confirmed, `reviewedSignature` `v1::largest_contrast:pathos::…`, and a final reload with the matching signature produced no warning. No horizontal overflow at 390×844 or 1440×900 (document/body `scrollWidth` equal to viewport; no element exceeded width); no handoff-related browser console errors and no Next.js error overlay across loads and interactions. Focused (56) and full (1176) `node --test` suites green.

**Resolved in commit:** `3f4484a` (matrix essay-direction engine) and the "Restore Module 3 matrix-change review on resumed later stages" commit on `observation-engine-redesign` (Module 3 handoff resume review); re-verified at `5de04f9`.
...
**Related:** WP-052, WP-080, WP-094; Prompt 16; Phase 6 / Epic H

**Walkthrough observation:** WP-094 established the shared success family on Modules 1, 6, 8, 9 and the completed dashboard behind a development gate. Modules 2, 3, 4, 5, and 7 still use legacy cards or multi-stage success UIs.

**Student-facing impact:** Students should meet one familiar success pattern across all nine modules, with module-specific truthful evidence, then see that same presentation in production without a visual gate.

**Why it matters technically or operationally:** Presentation-only generalization and promotion. Must not change progression, receipt authority, instructional rollout modes, or artifact schemas. No database migration.

**Recommended smallest reasonable fix:** Extend `successExperienceContract` builders for Modules 2/3/4/5/7; wire all nine routes + dashboard to the shared shell; remove `isSuccessExperienceFoundationEnabled`; align WP-052 3→4 CTA catalog with product.

**Gate retirement:** Delete development-only presentation gate after all nine routes pass acceptance; rollback is code/deploy only.

**Resolution notes:** (July 22, 2026)

- Builders: `buildModule2/3/4/5/7SuccessExperience` plus thin projections (`module2SuccessProjection`, `module7SuccessProjection`).
- Wired Modules 2–5 and 7 to `SuccessExperienceShell`; preserved mount CAS and primary destinations.
- WP-052: catalog `3-to-4` actionLabel → **Start Body Paragraph 1**; success clients assert shell + catalog destinations.
- Removed `isSuccessExperienceFoundationEnabled` and legacy gated branches on M1/6/8/9 + dashboard.
- Automated: `tests/wp095-success-system-generalize-promote.test.js` + updated wp094/wp052/wp080/module56 a11y (**83** related tests green).
...
### WP-099 — Teacher progress visibility foundation (Phase 7, development-only)

- **Module:** Teacher dashboard
- **Screen or area:** Teacher progress / submissions / settings; student detail
- **Issue type:** Architecture / UX
- **Severity:** High
- **Status:** Resolved
- **Related issues:** WP-080 (receipt truth); Prompt 20; Phase 7 teacher acceptance
- **Governing strategy:** §§1–2, 3.1–3.16, 4.1–4.5, §5, §6 journey language, Phase 7 teacher acceptance, §§8–9; cognitive-load charter; writing-artifact-decision-architecture
- **Precondition:** WP-098 Resolved (`e7a7d35`)
- **Problem:** Teachers see competing Phase 1/2 tables, rollout panels above the roster, activity-count “completions,” quiz-as-readiness, Doc-as-submit, and private Doc/PDF URLs on the roster. Dual APIs disagree on membership and completion. No assignment-scoped authoritative progress projection exists.
- **Student / teacher problem:** Teachers cannot trust who is where in the writing journey or what to do next without reconstructing truth from misleading signals.
- **Audit (Prompt 20):** Entry `app/modules/10/page.js` → `TeacherDashboard.js`. Dual truth: `/api/teacher/overview` (assignment-row; M8 final_ready ∧ PDF) vs `/api/teacher/dashboard` (email union; activity-count modules; PDF URL = Completed). Never server-merged. Misleading: activity completions, quiz readiness, checklist formatting, final_ready lock, Doc-as-submit, current_module over receipt, Phase 1/2 labels, private URLs on roster, `/student` full final_text. Auth: teacher role OK; scoping not assignment-filtered. A11y: drawers lack focus trap; rows not keyboard-activatable.
- **Intended outcome:** One authoritative assignment-scoped read model + pure journey/receipt projection; privacy-bounded roster; Progress / Submissions / Settings jobs; detail least-data with focus isolation—development-gated only.
- **Nonnegotiables:** No schema/migration by default; preserve notes/grading/settings/rollouts/auth/student data; production keeps legacy dashboard; no struggle inference; no private prose/URLs on roster.
- **Acceptance criteria:**
  1. Pure projection: receipt wins Submitted; legacy never certifies; no activity-count completion; attention explicit-only; journey M1–9.
  2. Roster API teacher-only, assignment-scoped; excludes prose/URLs/notes; auth 401/403; empty ≠ failure.
  3. Detail least-data; no default full final_text; Doc/PDF only in authorized detail.
  4. Gated Progress / Submissions / Settings UI; detail focus trap / Escape / restore; no student leak across opens.
  5. Dev fixtures via `/api/dev/panel`; never in prod chunks.
  6. Production gate-off: legacy dashboard; panel 404; no foundation markers in chunks.
  7. Notes/grading/settings contracts unchanged.
- **Implementation notes:** (July 23, 2026) Gate `isTeacherProgressVisibilityFoundationEnabled`; pure `teacherProgressProjection` + fixtures; `buildTeacherRosterReadModel` + `/api/teacher/roster` + `/api/teacher/student-progress`; `TeacherProgressFoundationDashboard` Progress/Submissions/Settings; legacy dashboard preserved under gate-off; panel actions `ensureDevTeacherRole` / `wp099TeacherProgressFixtures`.
- **Verification evidence:**
  - Automated: `tests/wp099-teacher-progress-projection.test.js` + `tests/wp099-teacher-roster-read-model.test.js` **18/18**.
  - Agent browser: `scripts/wp099-browser-acceptance.mjs` → **40/40** (390×844 + 1440×900 + keyboard + 200% zoom + detail isolation); `/tmp/wp099-browser/` not committed.
  - Production: `.next-wp099-prod`; server :3014; `/api/dev/panel` → 404; `/api/teacher/roster` → 404; legacy dashboard; chunk scan clean (`scripts/wp099-prod-acceptance.mjs` **5/5**).
- **Human remainder:** subjective teacher-workflow density / label fit only.
- **Resolved in commit:** `0389531`

---

### WP-100 — Production promotion of teacher progress visibility (Phase 7)

- **Module:** Teacher dashboard
- **Screen or area:** Production Progress / Submissions / Settings; student detail; roster APIs
- **Issue type:** Architecture / UX
- **Severity:** High
- **Status:** Resolved
- **Related issues:** WP-099 (foundation); WP-080 (receipt truth); Prompt 21
- **Governing strategy:** §§1–2, 3.1–3.16, 4.1–4.5, §5, §6, Phase 7, §§8–9; cognitive-load; artifact architecture
- **Precondition:** WP-099 Resolved (`0389531`)
- **Problem:** Accepted teacher progress foundation remains development-gated; production still serves legacy dual-truth dashboard; fixture paths remain on the server dependency graph; detail lacks assignment membership check and private/no-store caching.
- **Authorization confidence limit:** Schema supports global teacher role + assignment_name membership only—not teacher-to-class ownership. Production restricts to the current assignment; any teacher sees all students on that assignment. Follow-up only if product requires stronger isolation.
- **Intended outcome:** Authoritative roster + Progress/Submissions/Settings is the production default with no gate/DB mode; fixtures removed from prod graphs; legacy progress APIs retired; notes/grading/settings preserved; code-only rollback.
- **Acceptance criteria:**
  1. No `isTeacherProgressVisibilityFoundationEnabled`; production UI is the promoted dashboard with no legacy flash.
  2. Roster/detail APIs enabled for teachers in all environments; no fixture imports in prod API graphs/chunks.
  3. Auth 401/403 before student queries; assignment membership on detail; `Cache-Control: private, no-store`.
  4. Receipt precedence and least-data privacy preserved; no default final_text; no prose/URLs/notes on roster.
  5. Legacy `/dashboard`, `/overview`, `/student` removed or compatibility-routed without competing calculations.
  6. Notes/grading/settings contracts unchanged; no render-time writes.
  7. Real-data browser + denial + prod chunk scan pass; WP-100 evidence recorded.
- **Implementation notes:** (July 23, 2026) Promoted `TeacherProgressDashboard` as sole `TeacherDashboard`; deleted gate + legacy overview/dashboard/student routes; decoupled fixtures from builder; `requireTeacherSession` + no-store; assignment membership on detail; grading gated on durable receipt.
- **Verification evidence:**
  - Automated: `tests/wp099-*.test.js` + `tests/wp100-teacher-progress-production.test.js` **30/30**.
  - Agent browser (real DB roster): `scripts/wp100-browser-acceptance.mjs` → **39/39** (mobile+desktop, keyboard, 200% zoom); `/tmp/wp100-browser/` not committed. Live roster n=4 including submitted students.
...
- **Verification evidence:** Corrective release gate on `cdd3104115841d2c37794378822ffc6aec53f55d`: seedless `freshOk=true` + `a11yOk=true` (`docs/project-standards/walkthroughs/beta-matrix-results/cdd3104115841d2c37794378822ffc6aec53f55d.fresh-seedless.json`; Modules 1–8 + receipt + dashboard; `usedSeedThrough=false`, `usedDevPanel=false`); browser **67/67**; prod **4/4**; matrix **153/153 Pass**, gate ok. `FRESH.e2e_real_ui` = seedless ok (no limitation / no seedThrough).
...
- **Resolved in commit:** cdd3104115841d2c37794378822ffc6aec53f55d
...
- **Resolution notes:** (July 23, 2026) Closed with seedless evidence inside WP-101: `cdd310411….fresh-seedless.json` — Modules 1–9 receipt + dashboard; `seedless=true`, `usedSeedThrough=false`, `usedDevPanel=false`; keyboard families `a11yOk=true`. WP-101 Resolved on the same corrective gate (browser 67/67, prod 4/4, matrix 153/153).
