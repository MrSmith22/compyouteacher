...
| | |
|---|---|
| **Document role** | Living walkthrough / beta-readiness execution guide |
| **Branch freeze** | `observation-engine-redesign` @ `f04ddbf` |
| **Automated baseline** | `1154/1154` on `tests/*.test.js` (commit `f04ddbf`) |
| **Authoritative issue status** | [issue-log.md](issue-log.md) |
| **Next phase** | Manual beta-readiness sweep — **verification, not redesign** |

## Purpose

This document converts Jason's complete end-to-end walkthrough into an orderly plan that Cursor can execute **one student-facing problem at a time**.
...
The long-term product goal remains a reliable student writing processor. **This blueprint does not claim the application is market-ready, production-ready, legally cleared, accessibility-complete, or validated with target-age students.** Those gates require separate evidence.

## Implementation reconciliation status (2026-07-14)

- Checkpoint implementation (CP-A–H) and Module 1–3 matrix/argument work are **committed** on this branch.
- App-wide instructional patterns WP-048–062 are largely **code-present** but remain **Needs Verification** in the issue log.
- WP-063 planning-support fade is **Resolved** (`1b25ee2`).
- Live/manual acceptance during the implementation stream was intentionally **bounded**.
- Automatic suite green is **necessary but not sufficient** for beta-readiness.
- The next action is a **clean manual beta-readiness sweep**, then narrowly scoped FAIL follow-ups.
- Production-readiness and target-age validation remain **future gates**.

### Compact mapping (blueprint areas → evidence class)

| Area | Class |
|---|---|
| Module 1 durable completion, quiz feedback, first-task hierarchy (WP-078) | Implemented and automated; WP-078 historically browser-verified |
| Module 2 sources, evidence summary, rhetorical-strategy matrix, entry gate | Implemented and automated; **Needs current manual verification** |
| Module 3 matrix handoff, provenance/review, Evaluate/Claim/Thesis coaching | Implemented and automated (`f04ddbf`); **Needs current manual verification** |
| Module 4 provenance personalization / continuity | Implemented and automated (`ff19654`); **Needs current manual verification** |
| Module 5–6 outline/draft sequencing, gates, success polish | Implemented and automated (CP-F/G/H); **Needs current manual verification** |
| Module 6–7 planning vs writing representation (WP-001/065/066/063) | Implemented and automated; WP-001/065/066 historically browser-verified; WP-063 reconciled |
| Modules 8–9 Google Doc / APA / PDF / submission coaching (many WP-028–072) | Mixed: many **Resolved** with historical live checks; some **Needs Verification** (e.g. WP-047, WP-073–077) |
| App-wide WP-048–062 (hierarchy, disclosure, desk, color, rhythm, orientation) | Implemented for Modules 6–9 pilot surfaces; issue log **Needs Verification** |
| Accessibility / legal / public entry / production security / target-age study | **Still open / production readiness** |

Cursor must not implement the entire blueprint in one pass. Prefer issue-sized work after the sweep opens failures.

## Governing sources

Use these sources in this order:

1. The complete walkthrough PDF, `Edtech Essay Grader - Summer 26 Writing Processor.pdf` *(external / owner-held — not claimed to live in this repository)*
2. This blueprint
3. The current live application and repository
4. [Issue log](issue-log.md)
5. The Master Design Specification (`docs/project-standards/master-design-specification-phase-ii.md`)
6. [Cognitive Load Charter](../cognitive-load-charter.md) and [writing-artifact decision architecture](../writing-artifact-decision-architecture.md)

The walkthrough is the original usability evidence. This blueprint translates it into executable work. The current application determines what has already changed. The issue log tracks history and is the authoritative WP status source, but it is not assumed to contain every walkthrough observation.
...
Use this section as a **verified outcomes ledger**, not a license to rebuild working systems. Separate **code**, **automated tests**, **historical manual checks**, and **current re-verification still needed**.

### Architecture / checkpoints (code + automated)

- Module 1 durable completion and CAS progression (`a61c36d`); instructional quiz feedback (`f46be20`); first-task hierarchy WP-078 **Resolved** (historical live verification Jul 2026).
- Module 2 evidence summary, rhetorical-strategy matrix, and entry gate (`7e102f0`).
- Module 3 CP-D matrix handoff, provenance/review, Evaluate both-works readiness, Claim/Thesis coaching (`f04ddbf`).
- Module 4 CP-E provenance personalization (`ff19654`).
- Module 5–6 CP-F/G/H sequenced outline, drafting gates, success/a11y polish (`06bbb67`, `41cfa00`, `680f14d`).
- Planning vs writing representation: assembled prose stays prose-only; Module 6–7 writing labels; WP-063 fade schedule **Resolved** (`1b25ee2`).
- Automated suite at docs freeze: **`1154/1154`** @ `f04ddbf`.

### Historically browser-verified (do not assume still green without re-check)

- Generated essay prose no longer includes outline Roman numerals or headings (WP-001; re-confirmed during WP-065).
- Module 2 source persistence gating was repaired and verified.
- Saved source texts can be reopened during Module 3 analysis (WP-064).
- Module 6 and Module 7 writing surfaces use writing labels (WP-065, WP-066).
- Module 7 developer Unlock to Test restores editing (WP-070).
- Module 8 requires a current-visit Google Doc create/update before completion (WP-002 family).
- Module 8 Create versus Update wording reflects whether a document exists (WP-071).
- Module 8 completion uses the dedicated success page and advances `current_module` correctly (WP-067/068).
- Module 9 PDF / success guidance used successfully in prior live walkthroughs (WP-069 and related).
- Module 6 received substantial teacher-style coaching through WP-073 to WP-077 (**code present**; issue log still **Needs Verification** for several).

### Needs current manual verification (beta sweep)

- Fresh/reset and returning-student hydration across Modules 1→9.
- Module 2 matrix reasoning UX and Module 3 adopt/retain/review paths on real student work.
- Artifact continuity Module 3→7 on mobile **390×844** and desktop **1440×900**.
- App-wide WP-048–062 instructional contracts (hierarchy, disclosure, desk, color, rhythm, orientation).
- Google Doc create/update/recovery, APA, PDF, and final submit trust under current code.
- Accessibility basics, recovery/error states, horizontal overflow, stale/duplicate document checks.

Cursor must confirm the current repository and **issue-log status** before assuming Resolved. Historical PASS does not erase the need for a clean beta-readiness re-sweep.
...
**Historical note:** The walkthrough originally called this the most important unfinished instructional area. Substantial CP-D/E/F implementation has since shipped. Treat remaining Stage 2 work as **verification and FAIL-driven polish**, not as a blank redesign of Modules 3–5.
...
1. Read this blueprint and the [issue log](issue-log.md).
2. Prefer **FAIL follow-ups from the beta-readiness sweep** or an explicitly assigned WP — do not invent a broad redesign.
...
9. Update the issue log only after verification. **Needs Verification** is not **Resolved**.

## Next phase after this docs freeze

Implementation reconciliation through Module 3 CP-D and WP-063 is complete on `observation-engine-redesign` @ `f04ddbf`.

**Do not treat “start at M1.1” as the next command.** The first phase after importing/reconciling this blueprint is:

1. Freeze implementation changes except narrowly scoped FAIL fixes.
2. Confirm clean branch and automated baseline (`1154/1154` @ freeze commit).
3. Perform a bounded but complete student-path **manual beta-readiness sweep** (mobile 390×844 and desktop 1440×900).
4. Record PASS/FAIL evidence against the issue log.
5. Open narrowly scoped follow-up issues for failures.
6. Then decide whether the next **external beta** can begin.

Production-readiness (auth/privacy/legal/a11y completeness/target-age validation) remains a **later gate**.
