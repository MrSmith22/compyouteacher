...
/**
 * Rank derived observations into ≤3 primary comparative directions + custom.
 */
export function selectPrimaryPatternRecommendations(derivedResult) {
  ...
  if (contrasts.length) {
    // strongest contrast only (already max-tied; take first for primary)
    primary.push({
      ...contrasts[0],
      recommendationRank: 1,
      comparative: true,
    });
    supporting.push(...contrasts.slice(1));
  }

  const similarities = byKind("meaningful_similarity");
  if (similarities.length && primary.length < MAX_PRIMARY_RECOMMENDATIONS) {
    primary.push({
      ...similarities[0],
      recommendationRank: 2,
      comparative: true,
    });
    supporting.push(...similarities.slice(1));
  }

  // Combined dominant-across-works (not separate speech/letter cards)
  const dominants = byKind("dominant_per_work");
  if (dominants.length && primary.length < MAX_PRIMARY_RECOMMENDATIONS) {
    const speechDom = dominants.find((o) =>
      String(o.id).includes(":speech:")
    );
    const letterDom = dominants.find((o) =>
      String(o.id).includes(":letter:")
    );
    if (speechDom || letterDom) {
      const sAppeal = speechDom?.provenance?.appeals?.[0] || "appeal";
      const lAppeal = letterDom?.provenance?.appeals?.[0] || "appeal";
      const sRating =
        speechDom?.provenance?.ratings?.speech?.[sAppeal] ?? null;
      const lRating =
        letterDom?.provenance?.ratings?.letter?.[lAppeal] ?? null;
      primary.push({
        id: "combined_dominant_across_works",
        kind: "combined_dominant_across_works",
        label: "How each work’s strongest appeal shapes the comparison",
        why: [
          speechDom
          ...
          letterDom
            ...
            : null,
          "This could support an essay about how audience or purpose explains that difference or similarity.",
        ]
          .filter(Boolean)
          .join(" "),
        provenance: {
          ...
          appeals: [sAppeal, lAppeal].filter(Boolean),
        },
        recommendationRank: 3,
        comparative: true,
      });
    }
    supporting.push(...dominants);
  }

  // Remaining kinds → supporting observations
  ...
  // Cap primary at 3 (excluding custom)
