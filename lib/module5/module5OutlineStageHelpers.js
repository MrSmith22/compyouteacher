...
export function getModule5StagePresentation(stage, extras = {}) {
  ...
  const paragraphLabel = `Paragraph ${bodyIndex + 1}`;
      ...
      return {
        ...
        nextStepText:
          bodyIndex + 1 >= bodyCount
            ...
            : `Next you will review Paragraph ${bodyIndex + 2}.`,
...
export function moveOutlineBodyCard(body = [], fromIndex, toIndex) {
  ...
  return normalizeOutlineBodyOrder(next);
}
    ...
    case MODULE5_STAGE.FINALIZE: {
      ...
      for (let i = 0; i < list.length; i += 1) {
        ...
        if (!result.valid) {
          return {
            ok: false,
            message: `Paragraph ${i + 1}: ${result.message}`,
          };
        }
      }
  ...
  for (const card of saved) {
    ...
    if (savedSig && nextSig && savedSig !== nextSig) {
      changedSources.push({
        ...
        label: safeText(card.point) || safeText(card.bucket) || `Paragraph ${sourceIndex + 1}`,
      });
    }
  }
