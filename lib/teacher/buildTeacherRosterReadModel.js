/**
 * WP-099 — Assignment-scoped teacher roster read model (server builder).
...
export async function buildTeacherRosterReadModel(options = {}) {
  ...
  if (options.useFixtures) {
    const students = sortTeacherRosterRows(buildWp099SyntheticRosterRows());
    return {
      ok: true,
      ...
      submissions: students
        .filter((s) => s.receiptStatus?.submitted)
        .map((s) => ({
          studentId: s.studentId,
          email: s.email,
          displayName: s.displayName,
          submittedAt: s.receiptStatus.submittedAt,
  ...
  for (const assignment of assignments) {
    ...
    const mapped = mapBatchRowsToArtifactInput({
      ...
      email,
      displayName: null,
      vocabTransfer: vocabBy.get(email) || null,
      sources: sourcesMeta,
      ...
      exportedDoc: docRow
        ...
        : null,
      guidedApa: apaBy.get(email) || null,
...
/**
 * Least-data student detail for foundation path.
 * Omits default full final_text; Doc/PDF URLs only here; historical under labeled block.
