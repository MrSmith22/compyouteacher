...
export const CONNECT_BLOCKED_WRITE_NOTE =
...
export function getSourceContributionQuestions({
  ...
  const otherLabel = otherWorkLabel(otherSourceType);
  ...
  if (sourceType === "speech") {
    return [
      ...
      `Does it contribute something similar to the ${otherLabel} quotation, something different, or both?`,
    ];
  }
...
export function getConnectContinueHint({
  ...
  if (choice?.countsTowardProgress && !safeText(active?.note)) {
    return CONNECT_BLOCKED_WRITE_NOTE;
  }
