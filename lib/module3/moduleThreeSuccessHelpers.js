...
export const SUCCESS_ONE_WORK_NOT_READY =
  "Both works are not yet represented well enough to treat this argument as ready.";
...
export function buildModuleThreeSuccessSummary({
  ...
  const slice =
    evidenceArgumentSlice && typeof evidenceArgumentSlice === "object"
      ? evidenceArgumentSlice
      : null;
  const sliceReady = Boolean(
    slice?.argumentMapConfirmed &&
      safeText(slice?.thesisText || thesisText) &&
      sourceCounts.bothWorksVerified
  );

  const directionLabel =
    safeText(slice?.directionDescriptor?.label) ||
    safeText(pattern?.matrixProvenance?.selectedPatternLabel) ||
    safeText(pattern?.text) ||
    "";

  const directionFamily =
    safeText(slice?.directionDescriptor?.family) ||
    safeText(pattern?.matrixProvenance?.selectedPatternKind) ||
    "";
  ...
  const speechItem =
    validEvidence.find((item) => item.sourceType === "speech") || null;
  const letterItem =
    validEvidence.find((item) => item.sourceType === "letter") || null;
  ...
  return {
    ...
    evidence: {
      ...
      oneWorkNotReadyMessage: SUCCESS_ONE_WORK_NOT_READY,
      items: validEvidence,
      speechSummary:
    ...
    argumentMap: {
      available: Boolean(slice),
      ready: sliceReady,
      direction: directionLabel,
      family: directionFamily,
      familyLabel: FAMILY_LABEL[directionFamily] || "",
      speechPassage: speechItem?.quote || "",
      speechExplanation: speechItem?.note || "",
      letterPassage: letterItem?.quote || "",
      letterExplanation: letterItem?.note || "",
      thesis: safeText(slice?.thesisText) || thesisText,
      proofDirections: proofPlanItems.map((item) => item.text),
    },
    meta: {
      clusterId,
      ...
      workingClaim:
        typeof claimArtifact?.workingClaim === "string"
          ...
          : "",
      evidenceArgumentSlice: Boolean(slice),
    },
