...
export const MODULE1_QUIZ_V2 = Object.freeze([
  {
    ...
    correctFeedback:
      "Rhetoric is the art of effective persuasion in writing and speaking—choosing words and strategies to influence an audience.",
    incorrectFeedback:
      "Rhetoric is not volume, fiction writing, or Greek study alone. It is the art of effective and persuasive writing and speaking.",
  ...
  },
  {
    ...
    correctFeedback:
      "Ethos is about credibility—making the audience trust that the speaker or writer is reliable and worth listening to.",
    incorrectFeedback:
      "Humor and emotion are pathos, and data is logos. Ethos means establishing credibility and trustworthiness.",
    requiresOptionalVideo: false,
  },
  {
    ...
    answer: "Ethos",
    correctFeedback:
      "Uniforms and experience signal expertise, so the chef builds ethos—credibility—before asking the audience to trust their advice.",
    incorrectFeedback:
      "This example is ethos: credentials and professional presentation build trust. Pathos is emotion, and logos is evidence or reasoning.",
    requiresOptionalVideo: false,
  },
  {
    ...
    answer:
      "Sharing a touching story about a sick puppy to encourage donations",
    correctFeedback:
      "A touching story aims at feeling. Pathos persuades by connecting to the audience’s emotions and sense of urgency.",
    incorrectFeedback:
      "Logic, credentials, and data are other strategies. Pathos is the emotional story that encourages donations.",
    requiresOptionalVideo: false,
  },
  {
    ...
    correctFeedback:
      "Pathos works because feelings can motivate action quickly—when audiences care, they are more likely to respond.",
    incorrectFeedback:
      "Facts and credentials matter, but pathos is effective specifically because it connects to emotion and can create urgency.",
    requiresOptionalVideo: false,
  },
  {
    id: "logos_definition",
    ...
    correctFeedback:
      "Logos persuades with clear evidence and reasoning so the audience can follow why a claim makes sense.",
    incorrectFeedback:
      "Trust is ethos and feelings are pathos. Logos focuses on clear evidence and logical reasoning.",
    requiresOptionalVideo: false,
  },
  {
    ...
    answer: "Logos",
    correctFeedback:
      "A research study is evidence. The dentist uses logos by supporting the claim with data the audience can evaluate.",
    incorrectFeedback:
      "Showing a study is logos—evidence and reasoning. Ethos would be credentials; pathos would be an emotional appeal.",
    requiresOptionalVideo: false,
  },
  {
    ...
    answer:
      "The people a writer or speaker is trying to reach and persuade",
    correctFeedback:
      "Audience means the specific people you are trying to reach and persuade—your rhetorical choices should fit them.",
    incorrectFeedback:
      "Audience is not “everyone,” only the teacher, or dictionary work. It is the people the writer or speaker tries to persuade.",
    requiresOptionalVideo: false,
  },
  {
    ...
    answer:
      "What the writer or speaker wants the audience to understand, believe, or do",
    correctFeedback:
      "Purpose is the writer’s or speaker’s goal—what they want the audience to understand, believe, or do.",
    incorrectFeedback:
      "Purpose is not length, publishing, or sentence style. It is what the writer or speaker wants the audience to understand, believe, or do.",
    requiresOptionalVideo: false,
  },
  {
    id: "king_application",
    ...
    answer:
      "How King uses rhetorical choices to persuade different audiences for different purposes",
    correctFeedback:
      "These terms help you explain how King’s rhetorical choices fit different audiences and purposes in the speech and letter.",
    incorrectFeedback:
      "The goal is analysis of persuasion, not dates, memorization, or the optional video. Explain how King’s choices reach different audiences for different purposes.",
    requiresOptionalVideo: false,
  },
]);

/**
 * Resolve question-specific teaching feedback for a Module 1 quiz item.
 * @param {{ correctFeedback?: string, incorrectFeedback?: string } | null | undefined} item
 * @param {boolean} correct
 */
export function getModule1QuizTeachingExplanation(item, correct) {
  if (!item) return "";
  return String(
 ...
 * Score is always calculated server-side from the answer key — never trust a client score.
 * quiz_version / submitted_at are included for new rows; callers may strip if columns absent.
 * @param {{ userEmail: string, answers: string[], quiz?: { answer: string }[], quizVersion?: number, submittedAt?: string, clientScore?: unknown }} input
 */
export function buildQuizPersistencePayload(input) {
  ...
  return {
    ...
    // Explicitly ignore any client-asserted score.
    _ignoredClientScore: input.clientScore ?? null,
  };
}

/**
 * Score answers against the canonical key. Client scores are never used.
 */
export function scoreQuizAnswers(answers, quiz = getActiveQuiz()) {
  const normalized = normalizeQuizAnswers(answers);
  const items = Array.isArray(quiz) ? quiz : getActiveQuiz();
  const score = items.reduce((acc, q, i) => {
    const given = (normalized[i] || "").toLowerCase().trim();
    const expected = String(q?.answer || "").toLowerCase().trim();
    return acc + (given && given === expected ? 1 : 0);
  }, 0);
  const total = items.length || QUIZ_ITEM_COUNT;
  return {
    score,
    total,
...
/**
 * Detect an identical successful attempt (same version + answers) to avoid
 * unintended duplicate rows on Retry after a confirmed save.
 */
export function answersMatchQuizAttempt(row, answers, quizVersion = QUIZ_CONTENT_VERSION) {
  if (!row) return false;
  const rowVersion =
    typeof row.quiz_version === "number"
      ? row.quiz_version
      : typeof row.quizVersion === "number"
        ? row.quizVersion
        : null;
  // Pre-migration rows may omit quiz_version; still dedupe on identical answers.
  if (rowVersion != null && rowVersion !== quizVersion) return false;
  const a = normalizeQuizAnswers(answers);
  const b = normalizeQuizAnswers(row.answers);
