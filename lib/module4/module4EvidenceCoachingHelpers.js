...
export function formatAlreadyUsedLabel(paragraphNumbers = []) {
  ...
  if (nums.length === 1) return `Already used in Body Paragraph ${nums[0]}`;
  if (nums.length === 2) {
    return `Already used in Body Paragraph ${nums[0]} and Body Paragraph ${nums[1]}`;
  }
  const head = nums.slice(0, -1).map((n) => `Body Paragraph ${n}`);
  return `Already used in ${head.join(", ")}, and Body Paragraph ${nums[nums.length - 1]}`;
}
...
export function formatIntentionalReuseLabel(paragraphNumbers = []) {
  ...
  if (nums.length === 2) {
    return `Used intentionally in Body Paragraph ${nums[0]} and Body Paragraph ${nums[1]}`;
  }
  const head = nums.slice(0, -1).map((n) => `Body Paragraph ${n}`);
  return `Used intentionally in ${head.join(", ")}, and Body Paragraph ${nums[nums.length - 1]}`;
}
