...
import { isSectionVerticalSliceEnabled } from "../dev/isBodyParagraphVerticalSliceEnabled.js";
...
/** WP-082 — Whole-essay map roles (presentation only; no evidence gates on conclusion). */
export function buildEssaySectionMap({ bodyCount = 2 } = {}) {
  const bodies = [];
  for (let i = 0; i < Math.max(1, bodyCount); i += 1) {
    bodies.push({
      id: `body-${i}`,
      label: getBodyParagraphLabel(i),
      purpose: "Prove one part of the thesis with evidence and reasoning.",
    });
  }
  return [
    {
      ...
      purpose: "Give the reader context and arrive at the thesis.",
    },
    ...bodies,
    {
      id: "conclusion",
      ...
      purpose:
        "Return to the argument in fresh words, synthesize the body, and end with purpose.",
    },
  ];
}
...
export const REVIEW_STAGE_META = {
  [REVIEW_STAGE_MAP]: {
    ...
    primaryActionLabel: `Review ${getBodyParagraphLabel(0)}`,
  },
  [REVIEW_STAGE_PLANS]: {
...
export function buildFinalReviewPresentation({
  ...
  return {
    ...
    essaySectionMap: isSectionVerticalSliceEnabled()
      ? buildEssaySectionMap({
          bodyCount: wantThirdBucket === true ? 3 : 2,
        })
      : null,
...
export function getReviewTeacherGuidance({
  ...
  if (stage === REVIEW_STAGE_MAP) {
    return {
      ...
      nextStep: `Then review ${getBodyParagraphLabel(0)} closely—one plan at a time.`,
    };
  }
    ...
    return {
      ...
      nextStep:
        planIndex < planCount - 1
          ? `After this check, move to ${getBodyParagraphLabel(planIndex + 1)}.`
          : guidance.nextStep,
      planProgressLabel: `${getBodyParagraphLabel(planIndex)} of ${planCount}`,
    };
  }
