...
/**
 * WP-085 — When legacy prose exists and moves are empty, open advanced mode
 * with the intact paragraph. Never invent move text from the paragraph.
 * @param {unknown} moveState
 * @param {{ moveOrder: string[], legacyProse?: string }} opts
 */
export function hydrateLegacyProseAsAdvancedMoveState(moveState, opts) {
  const normalized = normalizeSectionMoveState(moveState, opts);
  const legacyProse = String(opts.legacyProse || "").trim();
  if (!legacyProse) return normalized;
  const hasMoveText = normalized.moveOrder.some((id) =>
    String(normalized.moves[id] || "").trim()
  );
  if (hasMoveText || normalized.advancedProse.trim()) {
    return normalized;
  }
  return {
    ...normalized,
    advancedMode: true,
    advancedProse: legacyProse,
  };
}
