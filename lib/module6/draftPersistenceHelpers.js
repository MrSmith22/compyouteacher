...
import { countEssayWords } from "../essay/essayWordCount.js";
...
/**
 * Forward navigation gate for a writing section.
 * Always evaluate the provided live sections array (newest textarea value),
 * never a stale prior render snapshot held only in React state.
 */
export function evaluateForwardNavigationGate({
  sections = [],
  draftIndex = null,
  isReviewStage = false,
} = {}) {
  ...
  const gate = evaluateSectionReadiness(
    Array.isArray(sections) ? sections[draftIndex] : ""
  );
  if (!gate.ok) {
    return {
      ok: false,
      ...
      charCount: gate.charCount,
      minimumChars: gate.minimumChars,
      gradesStyle: false,
    };
  }
  return {
...
/**
 * Resolve the newest prose sections for gating / navigate writes.
 * Prefer a synchronously maintained live ref over React state.
 */
export function resolveLiveDraftSections(liveSections, draftState) {
  if (Array.isArray(liveSections)) return liveSections;
  if (Array.isArray(draftState)) return draftState;
  return [];
}
...
export function wordCount(text) {
  // WP-084 — shared algorithm (Modules 6–8). Keep this name for existing imports.
