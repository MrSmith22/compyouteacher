import {
  getModule8DraftForUser,
  upsertModule8DraftForUser,
...
export async function GET() {
  try {
    ...
    const result = await getModule8DraftForUser(userEmail);
    if (!result.ok) return failedResultResponse(result);

    return okResponse({ data: result.data });
  } catch (err) {
    return serverErrorResponse("Module 8 draft read failed:", err);
  }
}
...
export async function POST(req) {
  try {
    ...
    const result = await upsertModule8DraftForUser({
      userEmail,
      full_text,
      final_text,
      revised,
    ...
    return serverErrorResponse("Module 8 draft save failed:", err);
  }
}
