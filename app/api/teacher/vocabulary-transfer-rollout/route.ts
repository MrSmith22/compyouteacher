...
import {
  getAssignmentVocabularyTransferRollout,
  upsertAssignmentVocabularyTransferRollout,
} from "@/lib/supabase/helpers/vocabularyTransferRolloutSettings";
import {
...
/**
 * Teacher/ops read of Module 1 vocabulary-transfer rollout (WP-091).
 */
export async function GET(request: Request) {
  ...
  const result = await getAssignmentVocabularyTransferRollout(assignmentId);
  return NextResponse.json(result);
}

/**
 * Teacher/ops write — changes presentation mode only; never deletes rebuilt lesson state.
 * Does not modify writing_spine_mode or evidence_argument_mode.
 */
export async function PATCH(request: Request) {
  ...
  const result = await upsertAssignmentVocabularyTransferRollout(
    body.mode ?? body.vocabularyTransferMode,
    {
