...
import {
  getAssignmentEvidenceArgumentRollout,
  upsertAssignmentEvidenceArgumentRollout,
} from "@/lib/supabase/helpers/evidenceArgumentRolloutSettings";
import {
...
/**
 * Teacher/ops read of evidence-to-argument rollout (WP-088).
 */
export async function GET(request: Request) {
  ...
  const result = await getAssignmentEvidenceArgumentRollout(assignmentId);
  return NextResponse.json(result);
}

/**
 * Teacher/ops write — changes presentation mode only; never deletes rebuilt artifacts.
 * Does not modify writing_spine_mode.
 */
export async function PATCH(request: Request) {
  ...
  const result = await upsertAssignmentEvidenceArgumentRollout(
    body.mode ?? body.evidenceArgumentMode,
    {
