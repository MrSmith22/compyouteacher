...
export default function ReadAloud() {
  ...
  async function save() {
    ...
    try {
      ...
      const { data: pub } = supabase.storage.from("student-audio").getPublicUrl(filename);

      const { data: userData } = await supabase.auth.getUser();
      const user_email = userData.user?.email ?? "unknown@example.com";

      const { error: dbErr } = await supabase.from("student_readaloud").insert({
        user_email,
        module: 7,
        blob_url: pub.publicUrl,
        duration_seconds: 0,
        notes: ""
      });
  ...
  return (
    <div className="max-w-xl mx-auto p-6 space-y-4">
      ...
      <p>Record yourself reading your draft. Play it back and add quick notes on what to improve.</p>

      <div className="flex gap-3">
        {!isRecording && <button onClick={start} className="px-4 py-2 rounded-2xl shadow">Start</button>}
        {isRecording && <button onClick={stop} className="px-4 py-2 rounded-2xl shadow">Stop</button>}
